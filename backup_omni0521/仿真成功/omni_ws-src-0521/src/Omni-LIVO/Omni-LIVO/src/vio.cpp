...
#pragma omp parallel for schedule(dynamic, 16) \
    ...
    for (int i = 0; i < total_cells; i++) {
        ...
        // 【新增修复】：务必更新最后观测帧ID，否则 cleanupOldVisualPoints 永远无效！
        cc_data.last_seen_frame_id = new_frame_->id_;  // <--- 加上这一行
        } 
    } 
...
void VIOManager::generateVisualMapPoints(const std::vector<cv::Mat>& imgs, std::vector<pointWithVar> &pg)
{
    ...
    if (grid_num.size() != (size_t)total_cells) {
        grid_num.resize(total_cells, TYPE_UNKNOWN);
    }
    if (scan_value.size() != (size_t)total_cells) {
        scan_value.resize(total_cells, 0.0f);
    }
    ...
    // Keep TYPE_MAP cells from sparse-map retrieval, but clear stale pointcloud candidates.
    for (int i = 0; i < total_cells; ++i) {
        if (grid_num[i] != TYPE_MAP) {
    ...
    for (int i = 0; i < total_cells; i++) {  
        ...
        try {
            ...
            // 【关键修复】：无论是否启用跨相机追踪，都必须设置last_seen_frame_id
            // 否则cleanupOldVisualPoints永远不会删除这些点，导致内存泄漏！
            pt_new->cross_cam_data_.last_seen_frame_id = new_frame_->id_;
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+:error, n_meas)
        ...
        for (int i_pt = 0; i_pt < total_points; i_pt++)
        {
            ...
            if (!pt) continue;
            if (i_pt >= (int)visual_submap->camera_ids.size() ||
                i_pt >= (int)visual_submap->warp_patch.size() ||
                i_pt >= (int)visual_submap->inv_expo_list.size()) {
                continue;
            }
...
#pragma omp atomic
        ...
        for (int i_pt = 0; i_pt < total_points; i_pt++) {
            if (i_pt >= (int)visual_submap->camera_ids.size() || i_pt >= (int)visual_submap->errors.size()) {
                continue;
            }
            int cam_idx = visual_submap->camera_ids[i_pt];
            if (cam_idx >= 0 && cam_idx < num_cams) {
        ...
        points.erase(
            std::remove_if(points.begin(), points.end(),
                [this, current_frame_id, &total_points_deleted](VisualPoint* pt) {
                    ...
                    // 【关键修复】改为 last_seen >= 0 以包含第一帧（frame_id=0）
