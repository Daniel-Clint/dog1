...
void LIVMapper::handleVIO() {
    ...
    const ros::Time viz_stamp = ros::Time::now();
    publish_odometry(pubOdomAftMapped, viz_stamp);
    if (pub_body_cloud_en && feats_undistort && !feats_undistort->empty()) {
        publish_cloud_body(pubLaserCloudBody, feats_undistort, viz_stamp);
    }
}
...
void LIVMapper::handleLIO() {
    ...
    *pcl_w_wait_pub = *laserCloudWorld;
    ...
    if (pub_body_cloud_en) {
        const ros::Time viz_stamp = ros::Time::now();
        publish_odometry(pubOdomAftMapped, viz_stamp);
        publish_cloud_body(pubLaserCloudBody, laserCloudFullRes, viz_stamp);
    } else {
...
void LIVMapper::publish_odometry(const ros::Publisher &pubOdomAftMapped, const ros::Time &stamp) {
    ...
    odomAftMapped.header.stamp = stamp.isZero() ? ros::Time::now() : stamp;
