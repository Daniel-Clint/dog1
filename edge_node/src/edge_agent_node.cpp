...
private:
    ...
    std::string latest_status_message_;
    bool has_latest_status_message_ = false;
    std::string latest_pose_message_;
    bool has_latest_pose_message_ = false;
    std::string latest_video_frame_message_;
    bool has_latest_video_frame_message_ = false;
    std::string latest_local_cloud_message_;
    bool has_latest_local_cloud_message_ = false;
    std::string latest_global_submap_message_;
    bool has_latest_global_submap_message_ = false;
    std::string latest_panorama_pose_message_;
    bool has_latest_panorama_pose_message_ = false;
    ...
    void enqueueWebSocketMessage(const json& payload, const std::string& text)
    {
        ...
        if (event_name == "status") {
            latest_status_message_ = text;
            has_latest_status_message_ = true;
            ...
            if (has_panorama) {
                latest_panorama_pose_message_ = text;
                has_latest_panorama_pose_message_ = true;
            } else {
                latest_pose_message_ = text;
                has_latest_pose_message_ = true;
            }
        } else if (event_name == "video.frame") {
            latest_video_frame_message_ = text;
            has_latest_video_frame_message_ = true;
        } else if (event_name == "localCloud") {
            latest_local_cloud_message_ = text;
            has_latest_local_cloud_message_ = true;
        } else if (event_name == "globalSubmap") {
            latest_global_submap_message_ = text;
            has_latest_global_submap_message_ = true;
        } else {
    ...
    void clearPendingWebSocketMessages()
    {
        ...
        latest_status_message_.clear();
        has_latest_status_message_ = false;
        latest_pose_message_.clear();
        has_latest_pose_message_ = false;
        latest_video_frame_message_.clear();
        has_latest_video_frame_message_ = false;
        latest_local_cloud_message_.clear();
        has_latest_local_cloud_message_ = false;
        latest_global_submap_message_.clear();
        has_latest_global_submap_message_ = false;
        latest_panorama_pose_message_.clear();
        has_latest_panorama_pose_message_ = false;
    }
    ...
    bool takeLatestWebSocketMessage(std::string& text,
                                    std::string& slot,
                                    bool& has_slot)
    {
        std::lock_guard<std::mutex> lock(ws_queue_mutex_);
    ...
    void flushPendingMessages(websocket::stream<tcp::socket>& ws)
    {
        ...
        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_status_message_, has_latest_status_message_)) {
            ws.write(asio::buffer(text));
        }

        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_pose_message_, has_latest_pose_message_)) {
            ws.write(asio::buffer(text));
        }

        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_panorama_pose_message_, has_latest_panorama_pose_message_)) {
            ws.write(asio::buffer(text));
        }

        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_video_frame_message_, has_latest_video_frame_message_)) {
            ws.write(asio::buffer(text));
        }

        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_local_cloud_message_, has_latest_local_cloud_message_)) {
            ws.write(asio::buffer(text));
        }

        if (ws_connected_.load() && !stop_ws_thread_.load() &&
            takeLatestWebSocketMessage(text, latest_global_submap_message_, has_latest_global_submap_message_)) {
            ws.write(asio::buffer(text));
        }
    }
